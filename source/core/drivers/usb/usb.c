...
dword PTR2LINEAR(void *ptr)
{
  	return (dword)FP_SEG(ptr) * 16L + (dword)FP_OFF(ptr);
}

dword LINEAR2PTR(void *ptr)
{
  	return (dword)FP_SEG(ptr) * 16L + (dword)FP_OFF(ptr);
}
...
unsigned long cdecl (*inportl)(unsigned port) =
	(unsigned long (*)(unsigned))g_inportl;
/*****************************************************************************
*****************************************************************************/
...
void cdecl (*outportl)(unsigned port, unsigned long val) =
	(void (*)(unsigned, unsigned long))g_outportl;
#endif
/*----------------------------------------------------------------------------
PCI CODE
...
/**/
	uint32_t td_linear, qh_linear, fl_linear, *fl;
	td_t *td;
	qh_t *qh;
	unsigned i;
...
/* init Frame List (FL) */
	...
	fl = (uint32_t *)LINEAR2PTR(fl_linear);
	for(i = 0; i < 1024; i++)
...
int uhci_control_transaction(unsigned adr,
		unsigned endpoint0_max_packet_size, void *setup_data_ptr,
		unsigned setup_len, void *data_ptr, unsigned data_len)
{
/* endpoint is always #0 for control transactions */
	static const unsigned endp = 0;
/**/
	unsigned char *setup_data = setup_data_ptr;
...
plus padding for memory alignment to 16-byte boundary */
	td_buf = kmalloc(num_tds * sizeof(td_t) + 15, 1);
	if(td_buf == NULL)
	{
...
/*****************************************************************************
*****************************************************************************/
static int usb_control_transaction(usb_dev_t *dev, void *setup_data_ptr,
		unsigned setup_len, void *data_ptr, unsigned data_len)
{
...
static int usb_get_descriptor(usb_dev_t *dev, unsigned desc_type,
		unsigned desc_index, unsigned desc_len, void *data)
{
	char setup[8];
...
/* allocate memory for configs */
	dev.configs = kmalloc(dev.num_configs * sizeof(usb_config_t));
	if(dev.configs == NULL)
MEM:	{
		printf("Error: out of memory\n");
		return -1;
...
Allocate memory and load it all. */
		...
		data2 = kmalloc(cdesc_len);
		if(data2 == NULL)
			goto MEM;
/* allocate memory for interfaces */
		c->iface = kmalloc(c->num_ifs * sizeof(usb_iface_t));
		if(c->iface == NULL)
		{
			free(data2);
			goto MEM;
...
/* allocate memory for endpoints */
				iface->endp = kmalloc(iface->num_endp *
					sizeof(usb_endp_t));
				if(c->iface == NULL)
				{
					free(data2);
